...
public class TreeNodeTool {
	...
	public StringBuffer make_ext_tree(List<TreeNode> node_list) {
		...
		//设置标题
		tree_buf.append("\t		title:'"+node_list.get(0).getAttributes().get("system_name")+"'");
		//设置根节点
		...
		//当为多个节点的时候，初始化一个没有没名字的根节点，然后把多个节点作为子节点设置到里面
			...
			tree_buf.append("\t		,root:{id:'"+node_list.get(0).getId().split("__")[0]+"',\n");
	...
	public StringBuffer make_ext_tree_node(List<TreeNode> node_list,StringBuffer buff) {
		for(int i=0;i<node_list.size();i++){
			TreeNode node = node_list.get(i);
			if(i>0)
				buff.append(",");
			buff.append("function(){\n"); //开始
			buff.append("\t\t	var node = new Ext.tree.TreeNode({id:'"+node.getId()+"',text:'"+node.getText()+"'});\n");
			//设置属性
			...
			//设置事件
			if(node.getClick_listener()!=null && node.getClick_listener().trim().length()>0){
				buff.append("\t\t 	node.on('click',"+node.getClick_listener()+");\n");
			}
			if(node.getRight_click_listener()!=null && node.getRight_click_listener().trim().length()>0){
				buff.append("\t\t	node.on('rightclick',"+node.getRight_click_listener()+");\n");
			}
			//设置子节点
			if(node.getChild_nodes()!=null && node.getChild_nodes().size() > 0){
	...
	public List<TreeNode> parse_to_tree_node_from_department(
			List<Department> department_list, boolean is_recursive) {
	...
	public List<TreeNode> parse_to_tree_node_from_menu(List<Menu> menu_list,
		...
		for (Menu m : menu_list) {
			...
			// 子节点
			List<Menu> temp_children_menu =  menu_service.getTheUserChildMenu(m,user);
