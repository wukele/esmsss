...
public interface IRole_menuDao {
	 ...
	 *            (t_role_menu) 分页查询
	 */
	List<Role_menu> getListForPage(Role_menu role_menu, int pageNo, int pageSize,
	...
	/**
	 * @param 角色菜单
	 *            (t_role_menu) 多条查询 查询指定角色拥有哪个系统下的菜单
	 */
	List<Role_menu> getRolesystemListNoPage(Role_menu role_menu);

	/**
	 * @param 角色菜单
	 *            (t_role_menu) 多条查询 查询指定角色指定系统下的菜单
	 */
	...
	void deleteRoleMenuByRole(Role role);

	// 马志永 修改start
	void deleteRoleMenuByMenu(Menu menu);

	// end
	List<Role_menu> getRoleMenus(Role role);
