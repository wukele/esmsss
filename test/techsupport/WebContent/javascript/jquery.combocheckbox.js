...
*/
jQuery.fn.combocheckbox = function(options)
{
	// Setting class
	var settings =
	{	
...
var _originalElementJQuery = jQuery(this);
			...
			var _containerDefaultStyle = "border-left: solid 1px #7f9db9;border-top: solid 1px #7f9db9; background:#fff;border-right: solid 1px #7f9db9;border-bottom: solid 1px #7f9db9;";
			var _containerEnforcedStyle = "padding:1; padding-left:2px;";
			var _dropDownListJQuery = null;
			var _dropDownListEnforcedStyle = "list-style-type:none; width:144px;min-height:15px;padding-top:0;margin:0;";
			var _dropDownListDefaultStyle = "cursor:default;padding:2px;background:#fff;border-right:solid 1px #000;border-bottom:solid 1px #000;border-left:solid 1px #000;border-top:solid 1px #000;overflow:auto";
			...
			var _dropDownButtonDefaultStyle = "width: 16px;height: 18px;color:#4d6185;font-family: 幼圆;font-size: 10px;cursor: default;text-align: center;";
			var _dropDownButtonEnforcedStyle = "background-repeat:no-repeat;float:right;";
			var _dropDownButtonDefaultUnselectedStyle = "padding-left:0px;padding-top:1px;width:15px;height:15px;background:#c1d2fb;border:0px;";
			var _dropDownButtonDefaultSelectedStyle = "padding-left:1px;width:12px;height:13px;background:#cae2fd;border:solid 1px #95ade5";
			...
			function buildDropDownItem(childJQuery)
			{
				...
				if (childJQuery.is('option'))
				{
					...
					if (dataItemClass)
					{						
						dataItemHTML = String.format("<li style='{0}' class='{1}'><input type='checkbox' style='width:16' />{2}</li>", dataItemStyle, dataItemClass, dataItemText);
					}
					else
					{
						dataItemHTML = String.format("<li style='{0}'><input type='checkbox' style='width:16' />{1}</li>", dataItemStyle, dataItemText);
					}
			...
			function recursivelyBuildList(parentJQuery, childrenOptionsJQuery)
			{
				...
				var buttonDiv = "<li style='text-align:right;'><input type='button' class='submitbutton' id='div_button_dict"+_originalElementJQuery.attr("id")+"' value='保存' /></li>";
				parentJQuery.append(buttonDiv);
			}
			...
			function buildDropDownList()
			{
				...
				if (originalElementChildrenJQuery.length > 0)
				{
					...
					//alert(_containerJQuery.offset().left);
					_dropDownListJQuery.find("ul").width(settings.width);
					recursivelyBuildList(_dropDownListJQuery.find("ul"), originalElementChildrenJQuery);
				}
...
//				});
			...
			function initialiseControl()
			{
				...
				if(settings.textId!=null&&settings.textId!=""){
					_valueDisplayJQuery.text($("#"+settings.textId).val());
					_valueDisplayJQuery.attr("title", $("#"+settings.textId).val());
				}
				//bindEvents();
			...
			//选中已选值
			function showDropDownListDiv(){
				var valueIdValue = $("#"+settings.valueId).val();
				if(valueIdValue!=null&&valueIdValue!=""){
					var valueArray = valueIdValue.split("|");
					var aryLength = valueArray.length;
					for(var i=0;i<aryLength;i++){
						var checkedLi = _dropDownListJQuery.find("li[dataValue='"+valueArray[i]+"']");
						checkedLi.find("input[type='checkbox']").attr("checked",true);
					}
				}
				_dropDownListJQuery.show();
			}
			...
			$(document).click(function(event){
				var target = (event.target) ? event.target : event.srcElement;
					...
					var ishave = false;
					var idValue = "div_"+_originalElementJQuery.attr("id");
					for(var i=0;i<parents.length;i++){
						if(parents.eq(i).attr("id")==idValue||
					...
					if(!ishave){
						_dropDownListJQuery.find("input[type='checkbox']").each(function(){
			...
			_valueDisplayContainerJQuery.click(function(){
				if(!_dropDownListJQuery.is(":visible")){
					showDropDownListDiv();
				}else{
					_dropDownListJQuery.find("input[type='checkbox']").each(function(){
			...
			$("#div_button_dict"+$(this).attr("id")).click(function(){
				var textAppend = "",valueAppend="";
				$(this).parent().parent().find("input[type='checkbox']").each(function(){
					if($(this).attr("checked")){
						if(valueAppend==""){
							valueAppend = $(this).parent().attr("dataValue");
							textAppend = $(this).parent().text();
						}else{
							valueAppend = valueAppend + "|" + $(this).parent().attr("dataValue");
							textAppend = textAppend + "|" + $(this).parent().text();
						}
					}
				});
				if(settings.textId!=null&&settings.textId!=""){
					$("#"+settings.textId).val(textAppend);
					$("#"+settings.valueId).val(valueAppend);
				}else{
					_originalElementJQuery.attr("title",textAppend);
					_originalElementJQuery.attr("value",valueAppend);
				}
				_valueDisplayJQuery.text(textAppend);
				_valueDisplayJQuery.attr("title", textAppend);
				_dropDownListJQuery.hide();
			});
		});
}

jQuery.fn.selectCheckBox = function(options){
	var params = {
	...
	if(params.code!=undefined&&params.code!=null&&params.code!=""){
		$.ajax({
			  ...
			  url: params.url,
			  async: false,
			  data: {dict_code:params.code,query_simplepin:params.query},
			  dataType:"json",
			  timeout:1000,
			  success: function(result){
				  var roptionsList=sourceSelect.get(0).options;
				  var roptions = roptionsList.length;
				  for (var a=0;a<roptions;a++){
						roptionsList.remove(0); 
				  }
				for(var i=0;i<result.ldata.length;i++){
					sourceSelect.get(0).options.add(new Option(result.ldata[i].display_name,result.ldata[i].fact_value));
				}
				sourceSelect.combocheckbox(params);
			  }
			});
	}else{
		sourceSelect.combocheckbox(params);
