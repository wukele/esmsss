...
//取系统下子菜单回调方法
function childrenMenuBack(json,tdParent){
	var resultTable = $(json.tabledata);
	resultTable.find("tr[ck=1]").find("img").attr("src","images/ckall.gif");
	resultTable.find("tr[ck=0]").find("img").attr("src","images/ckunall.gif");
	resultTable.find("tr[ck=-1]").find("img").attr("src","images/ckgray.gif");
	resultTable.find("tr").find("span:nth(1)").bind("click",spanClickEvent);
	tdParent.after(resultTable.find("tr"));
}
...
function spCk(tdObj){
	...
	var tdParent = jqTdObj.parent().parent();

	var trWay = tdParent.attr("way");
	var isLeaf = tdParent.attr("lf");
	if(trWay==null)trWay=2;
	if(tdParent.attr("cl")==null && isLeaf=="N"){
		$("#w_way").val(parseInt(trWay)+1);
		$("#w_systemcode").val(tdParent.attr("sd"));
		$("#w_menucode").val(tdParent.attr("id"));
		$("#w_ischecked").val(tdParent.attr("ck"));
		setParams("w_");
		jQuery.post("sysadmin/queryChildRoleMenu_menu.action",params,function(json){
			childrenMenuBack(json,tdParent);
		},"json"); 
		tdParent.attr("cl",1);
	}

	var trId = tdParent.attr("id");
	var scode = tdParent.attr("sd");
	var tdObjText = jqTdObj.text();
	if(scode!=trId){
		if(tdObjText=="＋"){
			jqTdObj.text("－");
...
function imCk(tdObj){
	...
	if(parentTrObj.attr("way")!=0 || parentTrObj.attr("cl")!=null){
		//系统阶段选中状态设置
	...
	var isLeaf = parentTrObj.attr("lf");	
	if(isLeaf=="N"){
		$("#w_way").val(trId);
		$("#w_systemcode").val(scode);
		$("#w_ischecked").val(check);
		setParams("w_");
		jQuery.post("sysadmin/sheZhiChildNode_menu.action",params,null,"json"); 
	}
}

//取角色菜单的回调方法
function testRoleMenuBack(json){
	var resultTableObj = $(json.tabledata);
	var tableObj = $("#RoleMenutable");
	...
	tableObj.find("tbody").html(resultTableObj.find("tr"));
...
function saveAll(){ 
	ajaxAddDivCeng();
	var systemCodeStr="",checkStr="";
	var trListClone;
	var entTable = $("#RoleMenuEntable").find("tbody");
	var menuTable = $("#RoleMenutable").find("tbody");
	entTable.find("tr").remove();
	var checkedTr = menuTable.find("tr[ck!=0][way=0]");
	var checkTr,trCheckState,trSystemCode,isLeaf;
	//得到选中的系统
	checkedTr.each(function(){
		systemCodeStr += $(this).attr("id");
		systemCodeStr += "|";
	});

	if(systemCodeStr.indexOf("|")>-1){
		systemCodeStr = systemCodeStr.substring(0,systemCodeStr.lastIndexOf("|"));
	}
	 
	$("#s_systemcode").val(systemCodeStr);

	//得到所有选中的菜单
	var checkedMenuTr = menuTable.find("tr[ck!=0][way!=0]");
	entTable.append(checkedMenuTr.clone());
...
function hideMenuPage(){
	jQuery.post("sysadmin/removeSessionMenu_menu.action",null,null,"json"); 
	hidePage();
}
</script>
<style type="text/css">
...
<input type="hidden" id="s_way" />
<input type="hidden" id="w_way">
<input type="hidden" id="w_systemcode">
<input type="hidden" id="w_menucode">
<input type="hidden" id="w_ischecked">
<table width="100%" border="0" cellpadding="0" cellspacing="0" align="center">
    <tr>
      <td align="left" class="title1">角色权限</td>
      <td align="right" class="title1"><a href="#" id="closeDiv" onclick='hideMenuPage();'class="close"></a></td>
    </tr>
</table>
<!--角色菜单开始-->
