...
Globalpar globalpar = new Globalpar();
			Globalpar temp = null;
			List globList = null;
			String top_parentdeptid_ba = "";
			globalpar.setGlobalparcode("top_parentdeptid_ba");
			globList = CacheManager.getCacheGlobalpar(globalpar);
			if(globList!=null&&globList.size()>0){
				temp = (Globalpar)globList.get(0);
				top_parentdeptid_ba = temp.getGlobalparvalue();	
			}
String level = "";
...
var parentdeptid = '<%=top_parentdeptid_ba%>';
 tree=new dhtmlXTreeObject("depart_treeDropDown","100%","100%",parentdeptid);
  ...
  tree.setXMLAutoLoading("<c:out value="${pageContext.request.contextPath}"/>/sysadminDefault/deptAllTreeWithDepartlevel_tree.action?level="+<%=level%>);
  tree.loadXML("<c:out value="${pageContext.request.contextPath}"/>/sysadminDefault/deptAllTreeWithDepartlevel_tree.action");
