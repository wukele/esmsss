...
pageUrld="<%=request.getContextPath() %>/icksystem/queryMoRenlist_st_device.action";
divnidd="tabledatt";
//tableidd="dictTable";
//loadPage("tabledatt");
...
$(document).ready(function() {	
	$("#t_query_simplepinfuwu").remove();
	$("#mybody_ylcsDrop").find("table:first").find("td:first").append("<input id='t_query_simplepinfuwu' class='inputstyle' type='text' style='width:98%' />");
	...
	if(gGxdwbm!=null && gGxdwbm!=""){
		$("#t_query_simplepinfuwu").before("<input type='hidden' id='t_gxdwbm' value='"+gGxdwbm+"' />");
	}

    $("#t_query_simplepinfuwu").before("<input type='hidden' id='t_hylbdm' value='"+dataid+"' />");
	
	$("#t_query_simplepinfuwu").focus();
...
function xuanzhongzhi(tr){
	...
	var dmva;
	if(gdictCode==undefined || gdictCode==null)
		dmva=$(tr).attr("id");
	else{
		if(gdictCode!="kcyzx"){
	...
	$("#" + gcodeValue).attr("value",$(tr).find("td").eq(1).text());
}
</script>
</head>
<body>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" background="images/toolbar.gif">
  <tr>
    	<td width="85%"  style="text-align:left"><input id='t_query_simplepinfuwu' class="inputstyle" type='text' style="width:98%" /><input id='t_zxbz' type='hidden' /></td>
