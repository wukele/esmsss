...
//var parentdeptid = "<c:out value='${parentdeptid}'/>";
  ...
  tree.setXMLAutoLoading("<c:out value="${pageContext.request.contextPath}"/>/basicsystem/tree2_xctb.action?hylbdmall="+hylbdmall);
  tree.loadXML("<c:out value="${pageContext.request.contextPath}"/>/basicsystem/tree2_xctb.action?hylbdmall="+hylbdmall);
  
 var csString = ""; //全部场所名称
 var cscode = "";   ///  场所信息"1111||场所1||J||营业性娱乐场所";
 ...
 function doOnCheck(itemId){
     ...
     if(flagcheck==1){
       ...
       if(note=='-'){
            ...
            var hylxcode = allString.substring(0,index);
            var hylxString = allString.substring(index+2);
       
	        var userData = "<Params>";
			userData += "<Param name='csmc'>"+csString+"</Param>";
			userData += "<Param name='csbm'>"+cscode+"</Param>";
			userData += "<Param name='hylxbm'>"+hylxcode+"</Param>";
			userData += "<Param name='hylxmc'>"+hylxString+"</Param>";
			userData += "</Params>";
  ...
  function selectQiYeRequest(userData){
  	jQuery.ajax({
			type: 'POST',
			url: "<c:out value="${pageContext.request.contextPath}"/>/basicsystem/treeQiye_xctb.action",
			data: userData,
			async: true,
			dataType: 'json',
  ...
  function selectItem(itemId){
       if(itemId!=''){ 
         var url ="<c:out value="${pageContext.request.contextPath}"/>/basicsystem/treeCs_xctb.action";
        //var pars = 'id='+ itemId;
  ...
  //点击清除按钮，清除调用框及相应隐藏框的值
	function addallcs(){
		//设置具体选择的行业类别
		var hylbString = tree.getSubItems(("0"));
		var hylbs = hylbString.split(",");
		var selectedHylbbm = "";
		var selectedHylbmc = "";
		$.each(hylbs, function(i, n){
  			if(tree.isItemChecked(n) == 0){
				if(selectedHylbbm == ""){
					selectedHylbbm = n.substring(1);
					selectedHylbmc = tree.getItemText(n);
				} else {
					selectedHylbbm = selectedHylbbm + ";" + n.substring(1);
					selectedHylbmc = selectedHylbmc + ";" + tree.getItemText(n);
				}
			}
		});
		sethylb(selectedHylbbm, selectedHylbmc);
		//设置企业名称，企业编码
        mcId = gmcId;
		dmId = gdmId;
