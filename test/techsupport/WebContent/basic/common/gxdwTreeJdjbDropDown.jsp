...
<table width="100%" border="0" cellpadding="0" cellspacing="0"  background="images/toolbar.gif">
    <tr>
    	<td width="85%"  style="text-align:left"><input id='t_query_simplepin' class="inputstyle3" type='hidden' style="width:98%" /></td>
...
//var parentdeptid = "<c:out value='${parentdeptid}'/>";
 tree=new dhtmlXTreeObject("depart_treeDropDown","100%","100%",0);
  ...
  tree.setOnClickHandler(doOnClick);
   tree.setXMLAutoLoading("<c:out value="${pageContext.request.contextPath}"/>/sysadminDefault/deptTreejdjb_tree.action?departmentFullCode="+gdictCode);
  //tree.loadXML("<c:out value="${pageContext.request.contextPath}"/>/privisys/department.do?method=tree&id=0");
  tree.loadXML("<c:out value="${pageContext.request.contextPath}"/>/sysadminDefault/deptTreejdjb_tree.action?departmentFullCode="+gdictCode);
