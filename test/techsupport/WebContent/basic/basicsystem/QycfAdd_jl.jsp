...
<script type="text/javascript">
	...
	function addVerify(){
		if (!checkControlValue("a_qycfid","Integer",-9999999999,9999999999,null,0,"奖励ID"))
		...
		if (!checkControlValue("a_ssdwmc","String",1,100,null,0,"所属单位名称"))
			return false;
		if (!checkControlValue("a_wgqkms","String",1,2000,null,1,"奖励原因"))
			return false;
		if (!checkControlValue("a_cfjgdm","Select",1,2000,null,1,"奖励结果"))
			return false;
		if (!checkControlValue("a_cfzxrxm","String",1,100,null,1,"奖励人"))
			return false;
		if (!checkControlValue("a_cfrq","Date",null,null,null,0,"奖励日期"))
			return false;
		//判断奖励日期不能大于今天
		if(!compare_now($("#a_cfrq").attr("value"),"奖励日期")){
   ...
   ////检查行业类别
    ...
    function changehylb(){
      if($("#a_hylbdm").attr("value")==null || $("#a_hylbdm").attr("value").length ==0 ){
       ...
       $("#a_cfjgdm").selectBox({code:"dm_qyjllb"});
      }else if($("#a_hylbdm").attr("value") == "C"){ //机修业 
...
<input type="hidden" id="a_cflb" value="奖励">
<input type="hidden" id="a_cflbbm" value="0">
