...
<script type="text/javascript">
	...
	function updatediv (json) { 
		...
		if(json.LQycf[0].hylbdm==null || json.LQycf[0].hylbdm.length ==0 ){
	       ...
	       $("#m_cfjgdm").selectBox({code:"dm_qycflb(jxy)"});
	...
	function modifyVerify(){
		if (!checkControlValue("m_qycfid","Integer",-9999999999,9999999999,null,0,"处罚ID"))
			return false;
		...
		if (!checkControlValue("m_ssdwmc","String",1,100,null,0,"所属单位名称"))
			return false;
		if (!checkControlValue("m_wgqkms","String",1,2000,null,1,"处罚原因"))
			return false;
		if (!checkControlValue("m_cfjgdm","Select",1,20,null,1,"处罚结果"))
			return false;
		if (!checkControlValue("m_cfzxrxm","String",1,100,null,1,"处罚人"))
			return false;
		if (!checkControlValue("m_cfrq","Date",null,null,null,0,"处罚日期"))
			return false;
			
		//判断处罚日期不能大于今天
		if(!compare_now($("#m_cfrq").attr("value"),"DATE")){
