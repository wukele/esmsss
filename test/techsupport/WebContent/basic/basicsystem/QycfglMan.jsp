...
$(document).ready(function() {
	...
	addHtml="basic/basicsystem/QycfAdd.jsp";
	addWidth="550";
	delUrl="basicsystem/delete_qycf.action";
	delid="d_qycfid";
	modHtml="basic/basicsystem/QycfModify.jsp";
	modUrl="basicsystem/modify_qycf.action";
	modWidth="550";
	detailHtml="basic/basicsystem/QycfDetail.jsp";
	...
	loadPage(divnid);
		...
		$("#p_djsj").datepicker();
		$("#p_hylb").selectBoxhylb();//行业类别	
		 
		//$("#p_cfjg").selectBox({code:"dm_cljg"});	
		$("#p_cfjg").selectBox({state:"dm_cljg"});
...
var qyjbxx_dataid;
var	JdccjdetailHtml="basic/publicsystem/QyjbxxDetail-gzth.jsp";
 function getjdccjqy(obj){
	qyjbxx_dataid = "qybm_"+$(obj).find("input[name='cs"+$(obj).attr("id")+"']").val(); //机动车拆解
	setWidth("ylcsjbxx_detail","850");
	setUrl("ylcsjbxx_detail",JdccjdetailHtml);
	bindDocument("ylcsjbxx_detail");
 }
  ...
  ///根据行业类别清空角色
	function qyjsChange(){
	  if($("#p_hylb").attr("value")==null || $("#p_hylb").attr("value").length==0 ){
	     ...
	     $("#p_cfjg").selectBox({code:"dm_qycflb(jxy)"});
		...
		<td class="tdbg">
			<table width="100%" border="0" cellspacing="0" cellpadding="2">
				<tr>
					...
					<td width="17%" class="pagetd">
						<input type="text" class="inputstyle" id="p_qymc" />
					</td>
					<td width="7%" class="pagedistd">
						企业编码
					</td>
					...
					<td width="18%" class="pagetd">
						<input type="text" class="inputstyle" id="p_ssdwmc" readonly />
					</td>
				</tr>
				<tr>
					<td width="7%" class="pagedistd">
					...
					<td width="17%" class="pagetd">
						<input type="text" class="inputstyle" id="p_cfrq" />
					</td>
					<td width="7%" class="pagedistd">
						至
					</td>
					<td width="17%" class="pagetd">
						<input type="text" class="inputstyle" id="p_cfrq2" />
					</td>
					<td width="7%" class="pagedistd">
						处罚结果
					</td>
