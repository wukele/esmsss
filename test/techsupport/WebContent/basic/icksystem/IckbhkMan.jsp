	...
	ickbhk_loadPage("tabledata");
		...
		$("#p_qymc").click(function(){
		    dataid=$('#p_hylbdm').val();
		 	...
		 	getTy_item_ymgn("p_qymc","p_qybm","","","","ickbhk");//页面功能 ickbhk:IC卡补换卡
		  });
...
function ickbhk_loadPage(divpageid){
...
function setPageList(pageno,url){	
	if (manVerify()){
		...
		var mygrid1 = $("#"+tableid).ingrid({ 
										...
										url: url,	
										onRowSelect: null,
										height: pageHeight-259,
										pageNumber: pageno,
										isPlayResultNull: true,
										colWidths: ["14%","6%","19%","4%","23%","12%","9%","8%","5%"]		
										});				
		}
		setHyxx();
}	
	function manVerify(){
		...
		if (!checkControlValue("p_cyzjdm","String",1,40,null,0,"证件类型"))
			return false;
		if (!checkControlValue("p_cjsjf","Date",null,null,null,0,"采集起始时间"))
			return false;
		if (!checkControlValue("p_cjsjt","Date",null,null,null,0,"采集截止时间"))
			return false;
		...
		if(cjsjf!=null && cjsjf!="" && cjsjt!=null && cjsjt!=""){
			if(cjsjf>cjsjt){
				jAlert("采集起始时间必须大于截止时间","验证信息",null,"p_cjsjf");
				return false;
			}
		}
		return true;
  ...
  <tr>
    <td class="tdbg"><table width="100%" border="0" cellspacing="0" cellpadding="2">
      ...
      <tr>
        ...
        <td width="23%" class="pagetd"><select name="select"  id="p_cyzjdm" value="">
        	<option></option>
        </select></td>
        ...
        <td colspan="1" class="pagetd">
				  <input type="text" class="inputstyle1" id="p_cjsjf" />至
				  <input type="text" class="inputstyle1" id="p_cjsjt" />
		</td>
      </tr>
      <tr>
        <td colspan="6" width="23%" class="pagetd"><table border="0" align="right" cellpadding="0" cellspacing="0">
            <tr>
