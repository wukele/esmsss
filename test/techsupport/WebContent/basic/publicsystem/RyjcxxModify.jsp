...
<script type="text/javascript">
	...
	function updatediv (json) { 
		...
		$("#m_rycfid").attr("value",setNull(json.LRyjcxx[0].rycfid));
	...
	function modifyVerify(){
		...
		if (!checkControlValue("m_cflb","String",1,100,null,0,"处罚类别"))
			return false;
		if ($("#m_cflbbm").val()==1&&!checkControlValue("m_qkms","String",1,2000,null,1,"违规情形"))
			return false;
		if (!checkControlValue("m_yjly","String",1,2000,null,0,"依据理由"))
			return false;
		if (!checkControlValue("m_jcfd","String",1,100,null,0,"奖惩幅度"))
			return false;
...
function updateback_rycf(json){
	if  (json.result=="success"){
		...
		cf_setPageList($("#pageNo").attr("value"));
...
<table width="100%" id="modify">
	<tr height="20">
		<td width="18%" class="red" id="jclb_add">
			奖惩类别
		</td>
		<td width="29%" class="detailtd">
			<select type="select" id="m_cflbbm">
			
			</select>
		</td>
		<td width="20%" class="red" id="jcmx_add">
			奖惩明细
		</td>
		<td width="33%" class="detailtd" >
			<select type="select" id="m_jcmxlbdm">
			</select>
		</td>
	</tr>
	<tr height="20">
