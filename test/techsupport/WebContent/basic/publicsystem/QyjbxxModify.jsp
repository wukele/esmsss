...
<script type="text/javascript">
	...
	function jxyInit(){ //机修
		...
		$("#bz_title").html("备注");
		if(qyjbxx_requestType=='biangeng'){
			setTrAppendTd('qyjbxxTr14','bgyy_');
		}
	}
	function ershoucInit(){ //二手车
		...
		//$("#bq_yyzt").selectBox({code:"dm_yyzt(ylcs)",width:"115px"});
		//$("#bq_zajb").selectBox({code:"dm_cszajb",width:"115px"});
		$("#m_sfazsxt").selectBox({code:"dm_bez",width:"115px"});
		//yyztOnchange();
		$("#hdrs_title").html("核定人数");
		$("#bz_title").html("备注");
		if(qyjbxx_requestType=='biangeng'){
	...
	function csfflInit(queryKey){
		if(selHylb=="J"){ //娱乐业专属
			...
			valueId:"m_qyfflbm",textId:"m_qyfflmc",width:364});
		}
	}
	function csflOnchange(){ //企业分类
		//先清空企业副分类
		$("#m_qyfflbm").attr("value","");
		$("#m_qyfflmc").attr("value","");
	...
	$("#m_qyfflbm").bind("propertychange",function(){ //企业副分类
		showOrHideByCsflAndJjlx();
	});
	$("#m_jjlxbm").bind("propertychange",function(){ //经济类型
		showOrHideByCsflAndJjlx();
	});
	function showOrHideByCsflAndJjlx(){ //用 企业分类-经济类型 判断显示与隐藏
		if(selHylb=="J"){ //娱乐业专属
			var theSecPageTag = 0;
			var jjlx = $("#m_jjlxbm").attr("value");
			//经济类型
	...
	function qyjbxx_updatediv(json){
		...
		$("#qyjbxxXiangxi").load("basic/publicsystem/QyjbxxXiangXiModify.jsp", {}, function(){
			$("#m_barq").datepicker();
			csflOnload();
			zajbOnchange();
			frdbzjOnchange();
			...
			csfflInit($("#m_qyzflbm").attr("value")); //显示企业副分类
