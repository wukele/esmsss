<%@ page language="java" pageEncoding="UTF-8"%>

		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		  <tr>
		    <td class="queryfont">&nbsp;企业基本信息</td>
		  </tr>
		  <tr>
		    <td valign="top"  class="tdbg">
		    <table width="100%" border="0" cellspacing="0" cellpadding="3" class="detail">
		      <tr height="25">
		        <td class="distd1" id="qymc_title">企业名称</td>
		        <td class="detailtd2" colspan="3"><span width="380px"  id="qymc_"></span></td>
		        <td class="distd1" id="qybm_title">企业代码</td>
		        <td width="132px" class="detailtd2"><span id="qybm_"></span></td>
		      </tr>
		      <tr height="25">
		      	<td class="distd1" id="jydz_title">企业地址</td>
		        <td  class="detailtd2" colspan="3" id="jydz_conte"><span width="380px" id="jydz_"></span></td>
		        <td class="distd1" id="qyzzjgdm_title">组织机构代码</td>
		        <td width="132px" class="detailtd2"><span id="qyzzjgdm_"></span></td>
		      </tr>
		      <tr height="25">
		      	 <td class="distd1" id="jyfwzy_title">经营范围</td>
		        <td  colspan="3" class="detailtd2"><span  width="380px"id="jyfwzy_"></span></td>
		       <td class="distd1" id="lxdh_title">联系电话</td>
		        <td width="132px" class="detailtd2"><span id="lxdh_"></span></td>
		      </tr>
		      <tr height="25">
		       <td width="12%" class="distd1" id="chzh_title">传真</td>
		        <td width="14%" class="detailtd2"><span id="chzh_"></span></td>
		        <td width="20%"class="distd1" id="yzbm_title">邮政编码</td>
		        <td width="20%" class="detailtd2"><span id="yzbm_"></span></td>
		        <td width="11%" class="distd1" id="kyrq_title">开业日期</td>
		        <td width="15%" class="detailtd2"><span id="kyrq_"></span></td>
		      </tr>
		      <tr height="25">
		        <td  class="distd1" id="jjlxmc_title">经济类型</td>
		        <td width="132px" class="detailtd2"><span id="jjlxmc_"></span></td>
		      	<td class="distd1" id="jymj_title">占地面积（平米）</td>
		        <td width="132px" id="jymj_conte" class="detailtd2"><span id="jymj_"></span></td>
		      	<td class="distd1" id="zczj_title">注册资本（万元）</td>
		        <td width="132px" class="detailtd2"><span id="zczj_"></span></td>
		      </tr>
		      <tr height="25">
		        <td class="distd1" id="frdb_title">法定代表人</td>
		        <td width="132px" class="detailtd2"><span id="frdb_"></span></td>
		        <td class="distd1" id="frdbzjlb_title">法人证件类型</td>
		        <td width="132px" class="detailtd2"><span id="frdbzjlb_"></span></td>
		        <td class="distd1" id="frdbzjhm_title">法人证件号码</td>
		        <td width="132px" class="detailtd2"><span id="frdbzjhm_"></span></td>
		      </tr>
		      <tr height="25">
		        <td class="distd1" id="frdblxfs_title">法人联系方式</td>
		        <td width="132px" class="detailtd2" id="frdblxfs_conte"><span id="frdblxfs_"></span></td>
		        <td class="distd1" id="babh_title" style='display:none;'>特行备案编号</td>
		        <td width="132px" class="detailtd2" style='display:none;'><span id="babh_"></span></td>
		        <td id="thbajg_title" class="distd1">特行备案管理机构</td>
			    <td id="thbajg_conte" class="detailtd2" width="132px"><span id="thbajg_"></span></td>
		      </tr>
		      <tr height="25">
		      	<td class="distd1" id="yyzzZjbh_title">营业执照编号</td>
		        <td width="132px" class="detailtd2"><span id="yyzzZjbh_"></span></td>
		        <td class="distd1" id="yyzzFzjg_title">营业执照<br>发证机关</td>
		        <td width="132px" class="detailtd2"><span id="yyzzFzjg_"></span></td>
		        <td class="distd1" id="swdjZjbh_title">税务登记证编号</td>
		        <td width="132px" class="detailtd2" id="swdjZjbh_conte"><span id="swdjZjbh_"></span></td>
 		      </tr>
		      <tr height="25">                               
		        <td class="zhehangtd" id="swdjFzjg_title">税务登记证<br>发证机关</td>
		        <td width="132px" class="detailtd2" id="swdjFzjg_conte"><span id="swdjFzjg_"></span></td>
			    <td class="distd1" id="jyxkZjbh_title">经营许可证编号</td>
		        <td width="132px" class="detailtd2" id="jyxkZjbh_conte"><span id="jyxkZjbh_"></span></td>
		       <td class="zhehangtd" id="jyxkFzjg_title">经营许可证<br>发证机关</td>
		        <td width="132px" class="detailtd2" id="jyxkFzjg_conte"><span id="jyxkFzjg_"></span></td>
		      </tr>
		      <tr height="25"> 
		        <td id="jyxkzfzjgdm_title" class="zhehangtd" style='display:none;'>行业许可证<br>发证机构代码</td>
			    <td id="jyxkzfzjgdm_conte" class="detailtd2" width="132px" style='display:none;'><span id="jyxkFzjgdm_"></span></td>
			     <td class="distd1" id="dwfzr_title">单位负责人</td>
		        <td width="132px" class="detailtd2"><span id="dwfzr_"></span></td>
		        <td class="distd1" id="dwfzrzjhm_title">单位负责人<br>身份号码</td>
		        <td width="132px" class="detailtd2"><span id="dwfzrzjhm_"></span></td>
		      </tr>
		      <tr height="25"> 
		        <td class="distd1" id="dwfzrlxfs_title">负责人联系方式</td>
		        <td width="132px" class="detailtd2"><span id="dwfzrlxfs_"></span></td>
		      	<td width="11%" class="distd1" id="bafzrxm_title">治安负责人</td>
			    <td width="132px" class="detailtd2" id="bafzrxm_conte"><span id="bafzrxm_"></span></td>
			    <td class="distd1" id='bafzrgmsfhm_title'>治安负责人身份证号</td>
				<td class="detailtd2" id='bafzrgmsfhm_conte' width="132px"><span id="bafzrgmsfhm_"></span></td>
		      </tr>
		      <tr height="25">
			     <td class="distd1" id='bafzrdh_title'>治安负责人联系方式</td>
				<td class="detailtd2" width="132px"id='bafzrdh_conte'><span id="bafzrdh_"></span></td>
		        <td id="bwbmdh_title"style='display:none;' class="distd1">保卫部门电话</td>
			    <td id="bwbmdh_conte"style='display:none;' class="detailtd2" width="132px"><span id="bwbmdh_"></span></td>	    
		      	<td id="sfazsxt_title"style='display:none;'  class="distd1">是否安装摄像头</td>
			    <td id="sfazsxt_conte"style='display:none;' class="detailtd2" width="132px"><span id="sfazsxt_"></span></td>
		      </tr>
		      <tr style='display:none;'>
			    <td id="azsxtsl_title"style='display:none;' class="distd1">摄像头数量</td>
			    <td id="azsxtsl_conte"style='display:none;' class="detailtd2" width="132px" colspan="5" ><span id="azsxtsl_"></span></td>
		      </tr>
		      <tr>
			    <td class="distd1" id="bz_title">备注</td>
			    <td colspan="5" width="870px" class="detailtd2" id="bz_conte"><span id="bz_"></span></td>
			  </tr>
		    </table></td>
		  </tr>
		</table>	
