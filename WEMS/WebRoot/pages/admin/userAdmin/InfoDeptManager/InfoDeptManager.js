...
Ext.onReady(function(){
	...
	var cm=new Ext.grid.ColumnModel([
		...
		{header:'部门ID',dataIndex:'deptNo',sortable:true},
		{header:'父级',dataIndex:'parentDeptNo',sortable:true,renderer:renderDeptName},
		{header:'名称',dataIndex:'deptName',sortable:true},
		{header:'描述',dataIndex:'comments',sortable:true},
		{header:'路径编码',dataIndex:'pathCode',sortable:true},
		{header:'部门类型',dataIndex:'deptType',sortable:true},
		{header:'本地编码',dataIndex:'localNet',sortable:true},
	...
	var  store=new  Ext.data.Store({
		...
		/*reader:new Ext.data.JsonReader({root:'infoDepts',totalProperty:'totalProperty'},[*/
		...
		idProperty : 'deptNo',
		sortInfo:{field:"deptNo",direction:"asc"}
	});
    ...
    [
                    ...
                    handler:function(){
                        ...
                        Ext.Ajax.request({
                            ...
                            success: function(response) {
                                ...
                                var ret=resp.ret;
                        		addForm.form.findField("deptNo").setValue(resp.infoDepts[0].deptNo);
                        		addForm.form.findField("deptName").setValue(resp.infoDepts[0].deptName);
                        		addForm.form.findField("parentDeptNo").setValue(resp.infoDepts[0].parentDeptNo);
                        		addForm.form.findField("parentDeptName").setValue(renderDeptName(resp.infoDepts[0].parentDeptNo));
                        		addForm.form.findField("comments").setValue(resp.infoDepts[0].comments);        
                        		addForm.form.findField("flag").setValue(resp.infoDepts[0].flag);
                        		addForm.form.findField("pathCode").setValue(resp.infoDepts[0].pathCode);
                        		addForm.form.findField("deptType").setValue(resp.infoDepts[0].deptType);
                        		addForm.form.findField("localNet").setValue(resp.infoDepts[0].localNet);
                        		addForm.form.findField("phone").setValue(resp.infoDepts[0].phone);
                        		addForm.form.findField("fax").setValue(resp.infoDepts[0].fax);
                        		addForm.form.findField("address").setValue(resp.infoDepts[0].address);
                        		addForm.form.findField("areaId").setValue(resp.infoDepts[0].areaId);
                        		addForm.form.findField("channelType").setValue(resp.infoDepts[0].channelType);
                        		addForm.form.findField("subChannelType").setValue(resp.infoDepts[0].subChannelType);
                        		addForm.form.findField("resChar1").setValue(resp.infoDepts[0].resChar1);
                        		addForm.form.findField("resChar2").setValue(resp.infoDepts[0].resChar2);
            ...
            '-',
            {
                    ...
                    handler:function(){                    
						...
						var paras={};
                    	paras["infoDept.deptNo"]=deptNo;
                    	ajaxSyncCall('deleteInfoDept.action',"infoDept.deptNo="+deptNo);
                    	paras=getFormParams(qryForm,"infoDept")
                    	paras.limit=10;
                    	paras.start=0;
                    	var allInfoDepts=ajaxSyncCall("selectInfoDept.action","infoDept.deptNo="+paras["infoDept.deptNo"]+"&"+"infoDept.deptName="+paras["infoDept.deptName"]).infoDepts;
	...
	var qryForm=new Ext.form.FormPanel({
		...
		items:[{
				...
				items:[{fieldLabel:'部门ID',name:'deptNo'}]
		},{
			columnWidth:.33,
			layout:'form',
			defaultType:'textfield', 
			items:[{fieldLabel:'部门名称',name:'deptName'}]
		},{
					...
					},
					{
						...
						items:[
							...
							},
							{
								...
								listeners:{
									focus:function(){
										var loader = new Ext.tree.TreeLoader({dataUrl: 'selectInfoDept.action'});
										...
										loader.processResponse = function(response, node, callback) {
										        ...
										        try {
										                ...
										                for(var i = 0,len = o.length; i< len; i++) {
										                		var _node={id:o[i].deptNo,text:o[i].deptName,deptNo:o[i].deptNo,deptName:o[i].deptName};
										...
										var win=new Ext.Window({
											...
											buttons:[{
												...
												handler:function(){
													...
													addForm.form.findField("parentDeptName").setValue(tree.getSelectionModel().getSelectedNode().attributes.deptName);
													win.close();
